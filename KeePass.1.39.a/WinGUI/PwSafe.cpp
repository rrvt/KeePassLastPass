...
BOOL CPwSafeApp::InitInstance()
{
  ...
  g_uThreadACP = GetACP();
    ...
    if((osvi.dwMajorVersion == 5) && (osvi.dwMinorVersion == 0))          // OS == Win2K
      g_bForceSimpleAsterisks = TRUE;
    if((osvi.dwMajorVersion <= 4) || ((osvi.dwMajorVersion == 5) && (osvi.dwMinorVersion == 0)))
      NewGUI_SetWin32Banner();                             // Restore old banner style OS < XP
  #else
    if (currentOS.is == CurrentOS::Win2K) g_bForceSimpleAsterisks = TRUE;         // OS == Win2K

    if (currentOS.is <= CurrentOS::Win2K) NewGUI_SetWin32Banner();// Restore old banner style OS <= Win2K
  #endif
