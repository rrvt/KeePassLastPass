...
void CAMSEdit::TimeBehavior::SetAMPM(bool bAM)
{
  ...
  CString strText;
  strText.Format(_T(" %s"), bAM ? m_strAM.GetBuffer() : m_strPM.GetBuffer());
...
TCHAR* ampm = ((CString&) strAMPM).GetBuffer();
    ...
    if (IsShowingSeconds())
  {
        if (IsShowing24HourFormat())
            ...
            strText.Format(_T("%02d%c%02d%c%02d %s"), nHour, m_cSep, nMinute, m_cSep, nSecond, ampm);
    }
  else
  {
        if (IsShowing24HourFormat())
            ...
            strText.Format(_T("%02d%c%02d %s"), nHour, m_cSep, nMinute, ampm);
    }
