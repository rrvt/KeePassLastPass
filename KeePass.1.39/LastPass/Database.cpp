...
DBentryP*  dbep;
  ...
  for (dbep = iter(lpe.url); dbep; dbep = iter++) {

    if (*dbep->dbe != lpe) continue;
...
DBentryP* DBIter::operator() (Dir rev) {
  urlSrch = false; url.clear();

  nEntry = kpdb->GetEntryCount();   i = rev ? nEntry-1 : 0;   return next(rev);
  }


DBentryP* DBIter::operator() (TCchar* url)
        {this->url = url; urlSrch = true;   nEntry = kpdb->GetEntryCount();   i = 0;   return next(Fwd);}


DBentryP* DBIter::operator++ (int) {i++; return next();}
DBentryP* DBIter::operator-- (int) {i--; return next();}


DBentryP* DBIter::next(Dir rev) {
int      delta = rev ? -1 : 1;
DBentry* dbe;

  for (; -1 < i && i < nEntry; i += delta) {

    if (urlSrch) {i = kpdb->Find(url, true, PWMF_URL, i);   if (i > nEntry) return 0;}
...
DBentryP* DBIter::current() {

  dbep.dbe = (DBentry*) kpdb->GetEntry(i);       if (!dbep.dbe) return 0;

  dbep.index = i; return &dbep;
  }


void DBIter::copy(DBIter& iter) {
  kpdb     = iter.kpdb;
  url      = iter.url;
  urlSrch  = iter.urlSrch;
  nEntry   = iter.nEntry;
  i        = iter.i;
  noteOnly = iter.noteOnly;
  dbep     = iter.dbep;
  }

#if 0
DBentryP* DBIter::startLoop() {i = -1;   nEntry = kpdb->GetEntryCount(); return nextEntry();}


DBentryP* DBIter::nextEntry() {
DBentry* dbe;
...
DBentryP* DBIter::startLoop(TCchar* url) {
...
bool DBentry::operator!=(DBentry& dbe) {
  if (uGroupId        !=    dbe.uGroupId)                   return true;
  ...
  if (strcmp(pszAdditional, dbe.pszAdditional))             return true;
  return false;
  }
